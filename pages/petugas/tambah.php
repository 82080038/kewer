...
if ($_POST) {
    ...
    if (!$username || !$password || !$nama || !$role) {
        ...
        if ($check) {
            ...
            if (!$error) {
                // Insert user
                $result = query("INSERT INTO users (username, password, nama, email, role, cabang_id) VALUES (?, ?, ?, ?, ?, ?)", [
                    $username, password_hash($password, PASSWORD_DEFAULT), $nama, $email, $role, $cabang_id ?: null
                ]);
