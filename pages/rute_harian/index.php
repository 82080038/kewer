...
$petugas_list = query("SELECT id, nama FROM users WHERE cabang_id = ? AND role IN ('petugas_cabang','petugas_pusat') AND status = 'aktif' ORDER BY nama", [$cabang_id]);
if (!is_array($petugas_list)) $petugas_list = [];

// Build route: get installments due on selected date for the petugas
$where = "a.cabang_id = ? AND a.jatuh_tempo <= ? AND a.status IN ('belum','telat')";
$params = [$cabang_id, $tanggal];
...
<body>
    ...
    <div class="container-fluid">
        <div class="row">
            <nav class="col-md-3 col-lg-2 d-md-block bg-light sidebar no-print">
                <div class="position-sticky pt-3">
                    <ul class="nav flex-column">
                        ...
                        <li class="nav-item"><a class="nav-link active" href="index.php"><i class="bi bi-map"></i> Rute Harian</a></li>
                    </ul>
                </div>
            </nav>
                    ...
                    <div class="card-body">
                        <form method="GET" class="row g-2 align-items-end">
                            <div class="col-md-3">
                                ...
                                <input type="date" name="tanggal" class="form-control form-control-sm" value="<?php echo $tanggal; ?>">
                            </div>
                            <?php if (!$is_petugas): ?>
                            <div class="col-md-3">
