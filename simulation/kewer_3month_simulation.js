...
const config = {
  ...
  credentials: {
    owner: { username: 'bos_simulasi', password: 'password123' },
    superadmin: { username: 'patri', password: 'password' },
    manager_pusat: { username: 'manager_pusat_sim', password: 'password123' },
    manager_cabang: { username: 'manager_cabang1', password: 'password123' },
    petugas: { username: 'petugas1_sim', password: 'password123' },
    karyawan: { username: 'karyawan1', password: 'password123' }
  }
};
...
async function login(page, role) {
  try {
    ...
    // Map role to actual credentials
    const roleUserMap = {
      'owner': config.credentials.owner,
      'superadmin': config.credentials.superadmin,
      'manager_pusat': config.credentials.manager_pusat,
      'manager_cabang': config.credentials.manager_cabang,
      'manager': config.credentials.manager_cabang,
      'petugas': config.credentials.petugas,
      'karyawan': config.credentials.karyawan
    };
    
    const creds = roleUserMap[role] || config.credentials.superadmin;
    
    // Use test_login endpoint for automated testing
    await page.goto(`${config.baseUrl}/login.php?test_login=true&username=${creds.username}&password=${creds.password}`, { waitUntil: 'networkidle2', timeout: 15000 });
    await delay(1000);
    ...
    if (page.url().includes('dashboard.php')) {
      ...
      // Try manual navigation if redirect didn't work
      ...
      throw new Error(`Not redirected to dashboard after login. Current URL: ${page.url()}`);
    }
  } catch (error) {
...
async function simulateDay(browser, dayNumber) {
  ...
  try {
    // Admin activities (daily) - use separate page
    const adminPage = await browser.newPage();
    try {
      await login(adminPage, 'admin');
      await adminPage.goto(`${config.baseUrl}/dashboard.php`);
      await delay(500);
    ...
    // Manager activities (daily) - use separate page
    const managerPage = await browser.newPage();
    try {
      await login(managerPage, 'manager');
      await managerPage.goto(`${config.baseUrl}/dashboard.php`);
      await delay(500);
    ...
    // Petugas activities (daily) - use separate page
    const petugasPage = await browser.newPage();
    try {
      await login(petugasPage, 'petugas');
      await petugasPage.goto(`${config.baseUrl}/dashboard.php`);
      await delay(500);
    ...
    // Karyawan activities (daily) - use separate page
    const karyawanPage = await browser.newPage();
    try {
      await login(karyawanPage, 'karyawan');
      await karyawanPage.goto(`${config.baseUrl}/dashboard.php`);
      await delay(500);
