...
class Nasabah {
    ...
    public function getById($id) {
        return $this->db->selectOne("
            ...
            LEFT JOIN db_alamat_simple.provinces p ON n.province_id = p.id
            LEFT JOIN db_alamat_simple.regencies r ON n.regency_id = r.id
            LEFT JOIN db_alamat_simple.districts d ON n.district_id = d.id
            LEFT JOIN db_alamat_simple.villages v ON n.village_id = v.id
            WHERE n.id = ?", [$id]);
    }
    ...
    public function create($data) {
        $sql = "INSERT INTO nasabah (kode_nasabah, nama, alamat, province_id, regency_id, district_id, village_id, ktp, telp, email, jenis_usaha, lokasi_pasar, foto_ktp, foto_selfie, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $params = [
            $data['kode_nasabah'],
            $data['nama'],
