...
switch ($method) {
    case 'GET':
        ...
        $sql = "SELECT dcr.*, c.nama_cabang, p.nama as prepared_by_nama, a.nama as approved_by_nama
                ...
                LEFT JOIN cabang c ON dcr.cabang_id = c.id
    ...
    case 'POST':
        ...
        // Check if reconciliation already exists for this date and cabang
        $existing = query("SELECT id FROM daily_cash_reconciliation WHERE cabang_id = ? AND tanggal = ?", 
                         [$cabang_id, $tanggal]);
        ...
        $result = query("INSERT INTO daily_cash_reconciliation 
                        (cabang_id, tanggal, kas_awal, total_penerimaan, total_pengeluaran, 
    ...
    case 'PUT':
        ...
        if ($input['status'] === 'approved' && $role !== 'manager' && $role !== 'owner' && $role !== 'superadmin') {
            http_response_code(403);
    ...
    case 'DELETE':
        ...
        // Only owner, superadmin, or manager can delete
