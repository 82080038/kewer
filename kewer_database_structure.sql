-- Kewer Database Structure Export
-- Portable version for installation on other computers
-- Database: kewer
-- Generated: 2026-04-16
--
-- INSTRUCTIONS FOR USE ON ANOTHER COMPUTER:
-- 1. Create a database named 'kewer' in MySQL/MariaDB
-- 2. Import this file: mysql -u root -p kewer < kewer_database_structure.sql
-- 3. Configure .env file with your database credentials (DB_HOST, DB_NAME, DB_USER, DB_PASS)
...
CREATE TABLE `role_permissions` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `role` enum('owner','superadmin','admin','manager','petugas','karyawan') NOT NULL,
...
CREATE TABLE `users` (
  ...
  `nama` varchar(100) NOT NULL,
  `email` varchar(100) DEFAULT NULL,
  `role` enum('owner','superadmin','admin','manager','petugas','karyawan') NOT NULL DEFAULT 'petugas',
  `cabang_id` int(11) DEFAULT NULL,
  `status` enum('aktif','nonaktif') NOT NULL DEFAULT 'aktif',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
...
-- Dump completed on 2026-04-16 19:01:32
